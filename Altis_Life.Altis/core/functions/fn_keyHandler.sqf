...
switch (_code) do
{
	...
	case 86:
	{
        ...
        {
            if((animationState cursorTarget) != "Incapacitated" && (currentWeapon player == primaryWeapon player || currentWeapon player == handgunWeapon player) && currentWeapon player != "" && !life_knockout && !(player getVariable["restrained",false]) && !life_istazed && !(player getVariable["surrender",false])) then
            {
	...
	//surrender (main sur la tete) shift + g
	case 34:
	{
		if(_shift) then {_handled = true;};
	...
	{
		if(_shift) then {
			if(vehicle player != player && (typeOf vehicle player) in ["B_Quadbike_01_F","C_Offroad_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","B_MRAP_01_F","B_MRAP_01_hmg_F","I_MRAP_03_F","I_MRAP_03_hmg_F","B_Heli_Light_01_F","C_Heli_Light_01_civil_F","B_Heli_Transport_01_F","I_Heli_light_03_unarmed_F","I_APC_Wheeled_03_cannon_F"]) then {
				if(!isNil {vehicle player getVariable "lights"}) then {
					if(playerSide == west) then {
						[vehicle player] call life_fnc_sirenLights;
					} else {
						if (playerSide == civilian) then {
							[vehicle player] call life_fnc_CivOrangeLights;
						} else {
	...
	case 80:
	{
		...
		if ((_shift) && (vehicle player == player)) then
		{
			cutText [format["Tranquille frangin...~"], "PLAIN DOWN"];
