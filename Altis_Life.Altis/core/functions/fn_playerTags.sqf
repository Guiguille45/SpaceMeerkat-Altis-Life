...
_headgear = ["H_ShemagOpen_tan","H_Shemag_tan","H_Shemag_olive_hs","H_Shemag_olive","H_ShemagOpen_khk"];
...

{
	...
	if(!(lineIntersects [eyePos player, eyePos _x, player, _x]) && {!isNil {_x getVariable "realname"}}) then
	{
		...
		if(count _sPos > 1 && {_distance < 15}) then
		{
			...
			{
				case ((goggles _x) in _goggles): {format["<t color='#000000'>Homme Cagoulé</t>"];};
				case ((headgear _x) in _headgear): {format["<t color='#000000'>Homme Cagoulé</t>"];};
				case ((uniform _x) in _uniform): {format["<t color='#000000'>Homme Cagoulé</t>"];};
