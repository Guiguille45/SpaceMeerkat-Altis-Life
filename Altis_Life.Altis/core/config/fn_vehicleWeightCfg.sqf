/*
	File: fn_vehicleWeightCfg.sqf
	Author: Bryan "Tonic" Boardwine
	
	Description:
	Master configuration for vehicle weight.
*/
private["_className"];
_className = [_this,0,"",[""]] call BIS_fnc_param;

switch (_className) do
{
	case "C_Offroad_01_F": {75};
	case "C_Offroad_01_repair_f": {100};
	case "B_G_Offroad_01_F": {75};
	case "B_G_Offroad_01_repair_F": {100};
	case "B_Quadbike_01_F": {25};
	case "C_Hatchback_01_F": {60};
	case "C_Hatchback_01_sport_F": {110};
	case "C_SUV_01_F": {70};
	case "C_Van_01_transport_F": {125};
	case "I_G_Van_01_transport_F": {125};
	case "C_Van_01_Fuel_F": {150};
	case "I_G_Van_01_fuel_F": {150};
	case "C_Van_01_box_F": {175};
	case "C_Boat_Civil_01_F": {85};
	case "C_Boat_Civil_01_police_F": {85};
	case "C_Boat_Civil_01_rescue_F": {85};
	case "B_Truck_01_box_F": {480};
	case "B_Truck_01_covered_F": {350};
	case "B_Truck_01_transport_F": {310};
	case "B_Truck_01_medical_F": {400};
	case "B_Truck_01_Repair_F": {430};
	case "B_Truck_01_fuel_F": {380};
	case "O_Truck_02_box_F": {280};
	case "O_Truck_02_fuel_F": {255};
	case "I_Truck_02_covered_F": {225};
	case "I_Truck_02_medical_F": {270};
	case "I_Truck_02_transport_F": {200};
	case "B_MRAP_01_F": {65};
	case "O_MRAP_02_F": {60};
	case "I_MRAP_03_F": {58};
	case "B_Heli_Light_01_F": {25};
	case "C_Heli_Light_01_civil_F": {25};
	case "O_Heli_Light_02_unarmed_F": {25};
	case "I_Heli_Transport_02_F": {50};
	case "O_Heli_Transport_04_F": {50};
	case "B_Heli_Transport_03_unarmed_F": {50};
	case "C_Rubberboat": {45};
	case "O_Boat_Armed_01_hmg_F": {175};
	case "B_Boat_Armed_01_minigun_F": {175};
	case "I_Boat_Armed_01_minigun_F": {175};
	case "B_G_Boat_Transport_01_F": {45};
	case "B_Boat_Transport_01_F": {45};
	case "O_Truck_03_transport_F": {350};
	case "O_Truck_03_covered_F": {375};
	case "O_Truck_03_repair_F": {480};
	case "O_Truck_03_fuel_F": {400};
	case "O_Truck_03_medical_F": {435};
	case "O_Truck_03_device_F": {400};
	case "B_Heli_Transport_01_camo_F": {50};
	case "B_Heli_Transport_03_F": {50};
	case "O_Heli_Transport_04_box_F": {50};
	case "O_Heli_Transport_04_repair_F": {50};
	case "O_Heli_Transport_04_covered_F": {50};
	case "Land_CargoBox_V1_F": {5000}; 
	case "Box_IND_Grenades_F": {350}; 
	case "B_supplyCrate_F": {700}; 
	default {-1};
};